...
export default {
  data () {
    return {
      headers: [
        ...
        { value: 'id', text: '아이디', sortable: false }
  ...
  methods: {
    subscribe (arrow) {
      ...
      const ref = this.$firebase.firestore().collection('boards').limit(limit).orderBy(order, sort)
      let query

      switch (arrow) {
        case -1: query = ref.endBefore(last(this.docs)).limit(limit)
