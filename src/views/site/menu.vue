  ...
  <v-list-item>
        ...
        <v-list-item-action>
          <v-btn @click="$store.commit('setEdit', !$store.state.editable)" icon>
            <v-icon v-text="$store.state.editable ? 'mid-eye' : 'mdi-pencil'"></v-icon>
          </v-btn>
        </v-list-item-action>
      </v-list-item>
      <v-divider></v-divider>
...
export default {
  ...
  methods: {
    ...
    openDialogSubItem (index, subIndex) {
      ...
      if (subIndex < 0) {
        this.formSubItem.title = 'mdi-crosshairs-question'
        this.formSubItem.to = ''
      } else {
    ...
    async saveSubItem () {
      if (this.selectedItemIndex < 0) {
        ...
        this.items[this.selectedItemIndex].subItems[this.selectedItemIndex].title = this.formSubItem.title
        this.items[this.selectedItemIndex].subItems[this.selectedItemIndex].to = this.formSubItem.to
      }
      this.save()
    },
