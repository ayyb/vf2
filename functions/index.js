...
admin.initializeApp({
  ...
  databaseURL: functions.config().admin.db_url// 'https://lee-vf2.firebaseio.com'
})
...
exports.deleteUser = functions.auth.user().onCreate(async (user) => {
  const { uid } = user
...
exports.incrementBoardCount = functions.firestore.document('boards/{bid}').onCreate(async (snap, context) => {
  try {
    await fdb.collection('meta').doc('boards').updata('count', admin.firestore.FieldValue.increment(1))
  } catch (e) {
